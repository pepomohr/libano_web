...
export default function BookingSection({ productCart, onBookingComplete }: Props) {
  ...
  // Generate 14 days starting today (skip Sundays)
  const availableDays = useMemo(() => {
    const days: Date[] = []
    let cursor = addDays(today, calendarOffset * 7)
    while (days.length < 14) {
      if (cursor.getDay() !== 0) days.push(cursor) // skip Sunday
    ...
    return (
      <main className="max-w-2xl mx-auto px-4 py-20 text-center">
        ...
        <p className="text-muted-foreground leading-relaxed mb-6">
          ...
          a las <strong className="text-foreground">{selectedTime}</strong>hs.
          Recibirás un recordatorio por WhatsApp.
        </p>
        <div className="bg-card border border-border rounded-md p-4 text-left space-y-2 mb-6">
          {pricing.lines.map((l) => (
            <div key={l.label} className="flex justify-between text-sm">
              <span className="text-muted-foreground">{l.label}</span>
              <span className="text-foreground font-medium">{formatPrice(l.price)}</span>
            </div>
          ))}
        ...
        <button
          ...
          className="px-6 py-2.5 bg-primary text-primary-foreground font-semibold rounded-md hover:bg-primary/90 transition-all"
        >
          Hacer otra reserva
        </button>
      </main>
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 py-10">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">Elegí fecha, servicios y confirmá en segundos.</p>
      </div>
      ...
      <div className="flex items-center gap-2 mb-8">
        {([1, 2, 3] as Step[]).map((s, i) => (
          <div key={s} className="flex items-center gap-2">
            ...
            {i < 2 && <div className={cn("flex-1 h-px w-8 sm:w-12", step > s ? "bg-primary/40" : "bg-border")} />}
          </div>
        ))}
      </div>
        ...
        <div className="lg:col-span-2">
          ...
          {step === 1 && (
            <div className="bg-card border border-border rounded-xl p-5 space-y-5">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-1">
                  <button
                    ...
                    className="p-1.5 rounded-md hover:bg-muted disabled:opacity-30 transition-all"
                  >
              ...
              {/* Days grid — 14 days, filling week slots */}
              <DaysGrid
                days={availableDays}
                    ...
                    {timeSlots.map((slot) => {
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            "py-2 text-sm rounded-md font-medium transition-all border",
                            booked
              ...
              <button
                ...
                onClick={() => setStep(2)}
                className="w-full py-3 bg-primary text-primary-foreground font-semibold rounded-md hover:bg-primary/90 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
              >
                Continuar
              </button>
            </div>
          ...
          {step === 2 && (
            <div className="bg-card border border-border rounded-xl p-5 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-semibold text-foreground">Elegí tus Servicios</h3>
              ...
              {selectedServices.includes("corte") && (
                <div className="flex items-center gap-2 bg-primary/10 border border-primary/30 rounded-md px-3 py-2 text-sm text-primary">
                  ...
                  Combo activo: Barba a $1.000 + Cejas GRATIS
                </div>
              )}
              ...
              <div className="space-y-3">
                {SERVICES.map((svc) => {
                  ...
                  if (isBarbaCombo) displayPrice = 1000
              ...
              {productCart.length > 0 && (
                <div className="bg-muted/50 rounded-lg p-3 text-sm">
                  <p className="font-medium text-foreground mb-1">Productos agregados</p>
                  {productCart.map((p) => (
                    <div key={p.productId} className="flex justify-between text-muted-foreground">
            ...
            <div className="bg-card border border-border rounded-xl p-5 space-y-4">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className={cn(
                      "w-full px-3 py-2.5 bg-input border rounded-md text-foreground placeholder:text-muted-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring",
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={cn(
                        "w-full pl-9 pr-3 py-2.5 bg-input border rounded-md text-foreground placeholder:text-muted-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring",
              ...
              <div className="bg-muted/50 rounded-lg p-3 text-sm space-y-1">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-foreground font-medium">{selectedTime}hs</span>
                </div>
              </div>

              <button
                onClick={handleConfirm}
                className="w-full py-3.5 bg-primary text-primary-foreground font-bold rounded-md hover:bg-primary/90 active:scale-[0.98] transition-all text-base shadow-lg shadow-primary/20"
              >
                Confirmar Reserva
              </button>
            </div>
          ...
          <div className="bg-card border border-border rounded-xl p-5 sticky top-24">
            ...
            {selectedDate && selectedTime && (
              <div className="mb-4 p-3 bg-primary/5 border border-primary/20 rounded-md">
                ...
                <p className="text-foreground font-medium capitalize text-sm">
            ...
            {pricing.lines.length > 0 ? (
              <div className="space-y-2 mb-4">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {l.tag && (
                        <span className={cn(
                          ...
                          l.tag === "REGALO" ? "bg-green-500/20 text-green-500" : "bg-primary/20 text-primary"
                        )}>
                          {l.tag}
                        </span>
                      )}
                    ...
                    <div className="text-right">
                      {l.original && (
                        <span className="text-xs text-muted-foreground line-through mr-1">
                          {formatPrice(l.original)}
                        </span>
                      )}
                      <span className={cn(
                        "font-medium",
                        l.tag === "REGALO" ? "text-green-500" : "text-foreground"
                      )}>
                        {l.tag === "REGALO" ? "GRATIS" : formatPrice(l.price)}
                      </span>
                    </div>
                  </div>
                ))}
                ...
                {(selectedServices.includes("barba") && selectedServices.includes("corte")) && (
                  <div className="text-xs text-primary/80 bg-primary/5 border border-primary/10 rounded px-2.5 py-1.5">
                    Ahorrás {formatPrice(5000 - 1000 + (selectedServices.includes("cejas") ? 2000 : 0))} con el combo
                  </div>
                )}
              </div>
            ) : (
              <p className="text-sm text-muted-foreground mb-4">
                Seleccioná servicios para ver el precio.
              </p>
            )}
          </div>
        </aside>
...
function DaysGrid({
  ...
  const startDow = firstDay.getDay() === 0 ? 7 : firstDay.getDay() // Mon=1 Sun=7
        ...
        return (
          <button
            ...
            className={cn(
              ...
              isSun ? "opacity-20 cursor-not-allowed" :
              isSel ? "bg-primary text-primary-foreground shadow-md shadow-primary/25" :
